...
void LCD_Display_String( u16 x , u16 y , u8 *STR , u16 color)
{
	while(*STR != '\0')
	{
		if(x>(240-char_width))//检测是否到了一行的最末尾，若是切换到第二行显示
		{
			x=0 ; 
			y+=char_height ; 
		}
		else if(y>(320-char_height))//检测是否到了最后一行，若是切换到始地址开始显示
		{
			x=0 ; 
			y=0; 
		}
		LCD_Display_char( x , y , *STR , color ) ; 
		x += char_width ;//显示完一个字符，列地址自动增加
		STR++ ; //显示的字符自增
	}
}
...
/****************************************************************************************
	*@brief		数字显示函数
	*@param		x,y,length是数字长度，color是数字的显示颜色
	*@retvel	无
*****************************************************************************************/
void LCD_Display_Num( u16 x , u16 y , u32 num , u16 color )
{
	...
	while(num)
	{
		LCD_Display_char( (x+char_width*(length--)) , y , (num%10)+'0' , color ) ;//从个位开始显示数字
		num /=10 ; 
	}
}
